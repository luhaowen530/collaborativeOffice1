<template>
    <div class="a2-needdealt">
        ...
        <needdealt-dialog :visible.sync="showDialog" :data="rowdata" @narrowDialog="narrowDialog" />
...
<script>
    ...
    import { getOperation } from "@/axios/a2-gateway"
    import exportOperation from "@/axios/a2-gateway"
    export default {
        components: {
            top,
            needdealtTable,
        ...
        created () {
            this.$emit("sidebarActive", "7");    // 向父组件传入当前页面的高亮索引，刷新页面保持当前高亮菜单不变
        ...
        methods: {
               ...
               getOperation().then(response => {
                    if(response.data.code == 200) {
                        console.log(response.data.data)
                            ...
                            this.old_dataList = response.data.data.list
            ...
            pagination() {   
                if(this.$refs.needdealtTop.topForm.processTitle != ''
                    ...
                    || this.$refs.needdealtTop.topForm.type != '') {
            ...
            searchTableList(obj){
                ...
                var arr = this.old_dataList.filter((item)=> {
                    return item.operateName.indexOf(obj.processTitle) != -1
                            && item.post.indexOf(obj.createdPerson) != -1
                            // && item.startTime.indexOf(obj.processStatus) != -1
                            && item.operateType.indexOf(obj.type) != -1
                })
                console.log(arr)
            ...
            //导出
            exportout() {
                window.location.href = exportOperation.exportOperation + "?operateName=" + this.$refs.needdealtTop.topForm.processTitle + "&post=" + this.$refs.needdealtTop.topForm.createdPerson + "&startTime=" + this.$refs.needdealtTop.topForm.processStatus + "&operateType=" + this.$refs.needdealtTop.topForm.type
            },
            // 接收table组件传来的查看事件
            handleLook(row){
                this.rowdata = row
