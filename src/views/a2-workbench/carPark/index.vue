...
<script>
    ...
    import { getCarpark,tosameCarpark } from "@/axios/a2-gateway"
    import exportCarpark from "@/axios/a2-gateway"
    export default {
        components: {
            top,
            needdealtTable,
        ...
        created () {
            this.$emit("sidebarActive", "4");    // 向父组件传入当前页面的高亮索引，刷新页面保持当前高亮菜单不变
        ...
        methods: {
            ...
            pagination() {   
                if(this.$refs.needdealtTop.topForm.processTitle != ''
                    ...
                    || this.$refs.needdealtTop.topForm.status != '') {
            ...
            searchTableList(obj){
                ...
                var arr = this.old_dataList.filter((item)=> {
                    return item.parkingName.indexOf(obj.processTitle) != -1
                            && item.parkingType.indexOf(obj.createdPerson) != -1
                            && item.contactName.indexOf(obj.processStatus) != -1
                            && item.contact.indexOf(obj.status) != -1
                })
                console.log(arr)
            ...
            //导出
            exportout() {
                window.location.href = exportCarpark.exportCarpark + "?parkingName=" + this.$refs.needdealtTop.topForm.processTitle + "&parkingType=" + this.$refs.needdealtTop.topForm.createdPerson + "&contactName=" + this.$refs.needdealtTop.topForm.processStatus
            },
