<template>
    <div class="a2-needdealt">
        ...
        <div class="needdealt-table">
            <needdealt-table  ref="multipleTable"  :dataList="dataList" :tableColumn="tableColumn" ifselect="true" @handleLook="handleLook" @handleDelete="handleDelete" />
        </div>
        <pagination v-show="total > 0" :total="total" :page.sync="currentPage" :limit.sync="pageSize" @pagination="pagination" />

        <needdealt-dialog :visible.sync="showDialog" :state="state" :data="propdata" :persondata="personList" :carparkdata="carparkList" @narrowDialog="narrowDialog" @refresh="refresh" />
...
<script>
    ...
    import { getArea,getAreaPerson,getCarpark,addArea,deleteArea } from "@/axios/a2-gateway"
    export default {
        components: {
            top,
            needdealtTable,
        ...
        data() {
            return {
                ...
                tableColumn: [
                    { name: '区域', prop: 'area'},
                    { name: '区域描述', prop: 'areaDetail'},
                    { name: '场站', prop: 'parking' },
        ...
        created () {
            this.$emit("sidebarActive", "5");    // 向父组件传入当前页面的高亮索引，刷新页面保持当前高亮菜单不变
            this.getTableList()
            this.getPersonList()
            this.getCarParkList()
        },
        methods: {
            ...
            getCarParkList(){
                ...
                getCarpark().then(response => {
                    if(response.data.code == 200) {
                        this.carparkList = response.data.data
                    }else {
            ...
            searchTableList(obj){
                ...
                var arr = this.old_dataList.filter((item)=> {
                    return item.area.indexOf(obj.processTitle) != -1
                            && item.parking.indexOf(obj.createdPerson) != -1
                            && item.staffName.indexOf(obj.processStatus) != -1
                })
                console.log(arr)
